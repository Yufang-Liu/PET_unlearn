    ...
    **Attributes**:
    ...
    def from_pretrained(cls, model, model_id, **kwargs):
        ...
        if peft_config.task_type not in MODEL_TYPE_TO_PEFT_MODEL_MAPPING.keys():
            ...
            if peft_config.task_type == "SEQ_MT_CLS":
                model = MODEL_TYPE_TO_PEFT_MODEL_MAPPING[peft_config.task_type](model, peft_config, kwargs['config'])
            else:
                model = MODEL_TYPE_TO_PEFT_MODEL_MAPPING[peft_config.task_type](model, peft_config)
...
class PeftModelForMultiTaskClassification(PeftModel):
    """
    Peft model for multi-task classification tasks.
    ...
    **Attributes**:
        ...
        ```
    """
    def __init__(self, model, peft_config: PeftConfig, config):
        super().__init__(model, peft_config)
        num_task = len(config['data']['multi_task'])
        self.modules_to_save = ["score"]
        for i in range(num_task):
            self.modules_to_save.append("classifier_list." + str(i))

        self.cls_layer_name_list = []
        for i in range(num_task):
            self.cls_layer_name_list.append("classifier_list." + str(i))
    ...
    def _prefix_tuning_forward(
        ...
        **kwargs,
        ...
        if "past_key_values" in fwd_params:
            ...
            for idx, classifier_name in enumerate(self.cls_layer_name_list):

...
@add_start_docstrings(
    """
    Bert Model transformer with multiple sequence classification/regression heads on top 
    ...
    def __init__(self, config, multi_task_config):
        ...
        for dataset_str in dataset_str_list:
            self.classifier_list.append(
                torch.nn.Linear(multi_task_config['model']['hidden_dim'],
                                multi_task_config['data'][dataset_str + '_num_class'],
                                device=multi_task_config['options']['device'])
            )
    ...
    def forward(
        ...
        for idx, classifier in enumerate(self.classifier_list):
            ...
            outputs = self.bert(
                ...
                token_type_ids=token_type_ids[task_indice]
                if token_type_ids is not None else None,
                position_ids=position_ids[task_indice]
                if position_ids is not None else None,
                head_mask=head_mask[task_indice]
